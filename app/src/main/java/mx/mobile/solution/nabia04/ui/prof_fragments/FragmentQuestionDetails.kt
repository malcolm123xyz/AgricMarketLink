...
import mx.mobile.solution.nabia04.data.entities.Reply
...
import mx.mobile.solution.nabia04.databinding.FragmentQuestDetailBinding
...
@AndroidEntryPoint
class FragmentQuestionDetails : BaseFragment<FragmentQuestDetailBinding>() {
    ...
    override fun getLayoutRes(): Int = R.layout.fragment_quest_detail
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        vb?.send?.setOnClickListener {
            val reply = vb?.message?.text.toString()
            if (reply.isEmpty()) {
    ...
    private suspend fun showQuestion() {
        question = viewModel.repository.getQuestion(questionId)
        vb?.from?.text = question.from
        vb?.date?.text = question.time
        vb?.question?.text = question.question
        vb?.upVote?.text = question.upVote
        vb?.downVote?.text = question.downVote
        vb?.area?.text = question.area
        ...
        vb?.questionViewItem?.removeAllViews()
