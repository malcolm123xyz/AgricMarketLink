...
import mx.mobile.solution.nabia04.ui.activities.MainActivity.Companion.databaseLoadingStatus
...
class DatabaseRepository(private val context: Context) {
    var birthDayPerson: mx.mobile.solution.nabia04.data.entities.EntityUserData? = null
    private val token: String?
    val filterString = "none"
    val filterType = 0
    private val TAG = "DatabaseRepository"
    private var allUserData: List<mx.mobile.solution.nabia04.data.entities.EntityUserData>? = null
    ...
    companion object {
        ...
        fun getBackEndDataObject(obj: mx.mobile.solution.nabia04.data.entities.EntityUserData?): DatabaseObject {
    ...
    init {
        dao = mx.mobile.solution.nabia04.data.MainDataBase.getDatabase(context).userDataDao()
        userFolioNumber = sharedP.getString(SessionManager.FOLIO_NUMBER, "") ?: ""
        token = sharedP.getString(SessionManager.LOGIN_TOKEN, "")
    }

    fun getAllData(): List<mx.mobile.solution.nabia04.data.entities.EntityUserData> {
        if (allUserData == null) {
            return ArrayList()
        }
        return allUserData!!
    ...
    fun insert(userDataList: mx.mobile.solution.nabia04.data.entities.EntityUserData?) {
    ...
    fun getUserData(id: String?): mx.mobile.solution.nabia04.data.entities.EntityUserData? {
    ...
    fun refreshDatabase(refresh: Boolean) {
        object : BackgroundTasks() {
            ...
            override fun onPreExecute() {
                databaseLoadingStatus.setValue(
                    mx.mobile.solution.nabia04.data.view_models.State(
                        true
                    )
                )
            }
    ...
    fun getEntity(list: List<DatabaseObject>): List<mx.mobile.solution.nabia04.data.entities.EntityUserData> {
        val entityUserDataList: MutableList<mx.mobile.solution.nabia04.data.entities.EntityUserData> =
            ArrayList()
        for (obj in list) {
            val u =
                mx.mobile.solution.nabia04.data.entities.EntityUserData()
            u.birthDayAlarm = obj.birthDayAlarm
            u.className = obj.className
            u.contact = obj.contact
            u.courseStudied = obj.courseStudied
    ...
    fun filter (query: String) {
        ...
        object: BackgroundTasks(){
            var filteredList: MutableList<mx.mobile.solution.nabia04.data.entities.EntityUserData> =
                ArrayList()
